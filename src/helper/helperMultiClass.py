...
def purity_score(y_true, y_pred):
    """
    Compute the purity score for a given set of true labels and predicted labels.
    ...
    for cluster in np.unique(y_pred):
        ...
        # Get true labels for samples in current cluster
        cluster_labels = y_true[cluster_indices]
        # Count occurrences of each true label in the current cluster
        label_counts = Counter(cluster_labels)
        # Get the majority class label in the current cluster
        majority_class = label_counts.most_common(1)[0][1]
        # Add the number of samples in the majority class to the purity score
        purity += majority_class
        
    # Normalize purity score by dividing by the total number of samples
...
def train_clfs_with_hyperparameters(model, param_name, param_values, X_train, y_train):
    ...
    for value in param_values:
        ...
        clf = model(**params, class_weight = 'balanced')
        clf.fit(X_train, y_train)
        clfs.append(clf)
